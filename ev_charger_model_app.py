...
with st.sidebar:
    ...
    line_km = st.slider("Grid line extension (km)", min_value=0.0, max_value=20.0, value=1.0, step=0.5)
    st.markdown('<div class="explainer">Distance to nearest transformer or substation. Longer runs increase civil & cable costs.</div>', unsafe_allow_html=True)
    ...
    electricity_sale_price_usd = st.number_input(
        "Electricity sale price ($/kWh)",
        min_value=0.01, max_value=2.0, value=0.15, step=0.01,
        format="%.2f",
        help="Price charged to customers in USD/kWh. This is the critical revenue lever."
    )
    inr_equiv = electricity_sale_price_usd * USD_TO_INR
    st.markdown(f'<div class="explainer">= ₹{inr_equiv:.2f}/kWh at ₹{USD_TO_INR}/$ exchange rate</div>', unsafe_allow_html=True)
...
with col_b:
    ...
    opex_labels = ["Energy cost", "Demand charges", "Land lease", "O&M / labour"]
    opex_vals   = [m["energy_cost_inr"], m["demand_cost_inr"], m["land_lease_inr"], m["maintenance_inr"]]
    opex_colors = [CORAL, AMBER, SLATE, TEAL]

    fig_opex = go.Figure(go.Bar(
        x=opex_labels,
        y=opex_vals,
        marker_color=opex_colors,
        text=[fmt_inr(v) for v in opex_vals],
        textposition="outside",
        cliponaxis=False,
        hovertemplate="%{x}<br>₹%{y:,.0f}<extra></extra>"
    ))
